...
namespace ns3
{
  ...
  int main(int argc, char *argv[])
  {
    ...
    consumerHelper.SetAttribute("Frequency", StringValue("10")); // 1 interests a seconds
    ...
    std::vector<std::string> list_names = read_prefixes();

    for (uint32_t i2 = 0; i2 < nLocality; i2++)
    {
      ...
      for (uint32_t i = 0; i < nLocality; i++)
      {
        ...
        prefix = list_names[i2];
        std::cout << "Consumer id " << i << " prefix " << prefix << std::endl;
    ...
    for (uint32_t ij = 0; ij < nLocality; ij++)
    {
      ...
      std::cout << "Producer id " << ij << " prefix " << prefix_prod << std::endl;
    ...
    Simulator::Stop(Seconds(5)); // We need to modify time of simulation as per our requirements. We can have more simulation time if we cants to test Ethereum or Docker Container.
