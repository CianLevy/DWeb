...
DWebConsumer::DWebConsumer()
    : max_metadata(10) // needed here to make sure when SetQ/SetS are called, there is a valid value of N
      ,
      m_q(0.7), m_s(0.7), m_seqRng(CreateObject<UniformRandomVariable>())
{
...
void
DWebConsumer::SendInterestOnNetwork(){
  if (!m_active)
    return;

  NS_LOG_FUNCTION_NOARGS();

  // sequence numbers are retained for consistency with standard ndnSIM simulations
  ...
  std::string oid = (*nameWithSequence)[-1].toUri(ndn::name::UriFormat::DEFAULT);
  NS_LOG_INFO("> Interest for " << oid);
