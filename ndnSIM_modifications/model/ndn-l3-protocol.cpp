...
L3Protocol::initialize()
{
  ...
  if (m_node){
    std::string name = Names::FindName(m_node);

    if (name.empty())
      name = boost::lexical_cast<std::string>(m_node->GetId());

    m_impl->m_forwarder = make_shared<::nfd::Forwarder>(*m_impl->m_faceTable, name);
  } 
  else
