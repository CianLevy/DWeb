...
Entry::insertOrUpdateInRecord(Face& face, const Interest& interest)
{
  ...
  if (it == m_inRecords.end()) {
    recordRequestTime(); // record new request
...
Entry::recordRequestTime(){
  ...
  m_requestTimes.push_back(currentTime);
  std::cout << "Count before " << m_requestTimes.size() << std::endl;
  for (auto i : m_requestTimes){
    std::cout << i.count() << " ";
  }
  std::cout << std::endl;


  // std::remove_if(m_requestTimes.begin(), m_requestTimes.end(), [&currentTime](std::chrono::nanoseconds i){ return currentTime - i > std::chrono::seconds{5}; });
  std::cout << "Count " << m_requestTimes.size() << std::endl;
  for (auto i : m_requestTimes){
    std::cout << i.count() << " ";
  }
  std::cout << std::endl;
...
int
Entry::getRequestRate(){
  int res = 0;
  
  // to do: this should be rounded and should also check the diff is 1 second 
  // before trying division to avoid divide by 0
  if (m_requestTimes.size() > 1)
    res = m_requestTimes.size() / ((m_requestTimes.back() - m_requestTimes.front()) / std::chrono::seconds{1});
  
  std::cout << "Request rate: " << res << std::endl;
  return res;
}
